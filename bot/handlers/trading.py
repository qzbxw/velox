...
async def cb_stats(call: CallbackQuery):
    ...
    for wallet in wallets:
        ...
        if stats:
            total_wins += stats["wins"]; total_loss += stats["losses"]; total_gp += stats["gross_profit"]; total_gl += stats["gross_loss"]
    ...
    kb = InlineKeyboardBuilder(); kb.button(text=_t(lang, "btn_pnl"), callback_data=f"cb_pnl:{context}"); kb.button(text=_t(lang, "btn_back"), callback_data=back_target); kb.adjust(1)
...
async def cb_fills(call: CallbackQuery):
    ...
    for wallet in wallets:
        ...
        for f in fills: f['wallet'] = wallet
...
async def cb_risk_check(call: CallbackQuery):
    ...
    for wallet in wallets:
        ...
        if "marginSummary" in state:
            ms = state["marginSummary"]; util = (float(ms.get("totalMarginUsed", 0)) / float(ms.get("accountValue", 0)) * 100) if float(ms.get("accountValue", 0)) > 0 else 0
            if util > 70: risky_positions.append(f"⚠️ <b>Wallet {wallet[:6]}...</b> Margin Usage: {util:.1f}%")
        for p in state.get("assetPositions", []):
            pos = p.get("position", {}); szi, coin_id, liq = float(pos.get("szi", 0)), pos.get("coin"), float(pos.get("liquidationPx", 0) or 0)
            if szi == 0 or liq <= 0: continue
            ...
            if mark > 0:
                ...
                if dist < 10: risky_positions.append(f"🚨 <b>{sym}</b> {'LONG' if szi > 0 else 'SHORT'} [{wallet[:4]}..]\n   Price: {pretty_float(mark)} | Liq: {pretty_float(liq)}\n   Buffer: <b>{dist:.2f}%</b>")
...
async def calc_start(call: CallbackQuery, state: FSMContext):
    ...
    kb = InlineKeyboardBuilder(); kb.button(text=_t(lang, "calc_spot_btn"), callback_data="calc_mode:spot"); kb.button(text=_t(lang, "calc_perp_btn"), callback_data="calc_mode:perp"); kb.button(text=_t(lang, "calc_reverse"), callback_data="calc_mode:reverse"); kb.button(text=_t(lang, "btn_back"), callback_data="sub:trading"); kb.adjust(2, 1, 1)
    await state.update_data(menu_msg_id=call.message.message_id); await call.message.edit_text(_t(lang, "calc_mode"), reply_markup=kb.as_markup(), parse_mode="HTML"); await state.set_state(CalcStates.mode)
...
async def calc_set_mode(call: CallbackQuery, state: FSMContext):
    ...
    if mode == "spot": await state.update_data(side="long"); await call.message.edit_text(_t(lang, "calc_balance"), reply_markup=_back_kb(lang, "calc_start"), parse_mode="HTML"); await state.set_state(CalcStates.balance)
    elif mode == "reverse": await call.message.edit_text("🛡️ <b>Reverse Risk Calculator</b>\n\nEnter <b>Entry Price</b>:", reply_markup=_back_kb(lang, "calc_start"), parse_mode="HTML"); await state.set_state(CalcStates.entry)
    else:
        kb = InlineKeyboardBuilder(); kb.button(text=_t(lang, "calc_long"), callback_data="calc_side:long"); kb.button(text=_t(lang, "calc_short"), callback_data="calc_side:short"); kb.button(text=_t(lang, "btn_back"), callback_data="calc_start"); kb.adjust(2, 1)
        await call.message.edit_text(_t(lang, "calc_side_msg"), reply_markup=kb.as_markup(), parse_mode="HTML"); await state.set_state(CalcStates.side)
    await call.answer()
...
async def calc_set_side(call: CallbackQuery, state: FSMContext):
    await state.update_data(side=call.data.split(":")[1]); await call.message.edit_text(_t(lang := await db.get_lang(call.message.chat.id), "calc_balance"), reply_markup=_back_kb(lang, "calc_start"), parse_mode="HTML"); await state.set_state(CalcStates.balance); await call.answer()
...
async def calc_set_balance(message: Message, state: FSMContext):
    ...
    try:
        await state.update_data(balance=float(message.text.replace(",", "."))); try: await message.delete()
        except Exception: pass
        if data.get("menu_msg_id"): await message.bot.edit_message_text(chat_id=message.chat.id, message_id=data["menu_msg_id"], text=_t(lang, "calc_entry"), reply_markup=_back_kb(lang, "calc_start"), parse_mode="HTML")
        else: await message.answer(_t(lang, "calc_entry"), reply_markup=_back_kb(lang, "calc_start"), parse_mode="HTML")
...
async def calc_set_entry(message: Message, state: FSMContext):
    lang, data = await db.get_lang(message.chat.id), await state.get_data()
    try:
        await state.update_data(entry=float(message.text.replace(",", "."))); try: await message.delete()
        except Exception: pass
        if data.get("menu_msg_id"): await message.bot.edit_message_text(chat_id=message.chat.id, message_id=data["menu_msg_id"], text=_t(lang, "calc_sl"), reply_markup=_back_kb(lang, "calc_start"), parse_mode="HTML")
        else: await message.answer(_t(lang, "calc_sl"), reply_markup=_back_kb(lang, "calc_start"), parse_mode="HTML")
...
async def calc_set_sl(message: Message, state: FSMContext):
    lang, data = await db.get_lang(message.chat.id), await state.get_data()
    try:
        await state.update_data(sl=float(message.text.replace(",", "."))); prompt = "💰 Enter <b>Risk Amount ($)</b> (e.g. 50):" if data.get("mode") == "reverse" else _t(lang, "calc_tp")
        try: await message.delete()
        except Exception: pass
        if data.get("menu_msg_id"): await message.bot.edit_message_text(chat_id=message.chat.id, message_id=data["menu_msg_id"], text=prompt, reply_markup=_back_kb(lang, "calc_start"), parse_mode="HTML")
        else: await message.answer(prompt, reply_markup=_back_kb(lang, "calc_start"), parse_mode="HTML")
...
async def calc_set_tp(message: Message, state: FSMContext):
    lang, data = await db.get_lang(message.chat.id), await state.get_data()
    try:
        val = float(message.text.replace(",", ".")); await state.update_data(tp=val); try: await message.delete()
        except Exception: pass
...
async def _render_funding_page(bot, chat_id, page=0, edit=False, msg_id=None):
    ...
    kb.row(*row); kb.row(InlineKeyboardButton(text=_t(lang, "btn_back"), callback_data="cb_menu"))
    if edit and msg_id:
        try: await bot.edit_message_text(chat_id=chat_id, message_id=msg_id, text=msg_text, reply_markup=kb.as_markup(), parse_mode="HTML"); return
        except Exception: pass
