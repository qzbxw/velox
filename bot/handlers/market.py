...
async def cb_market(call: CallbackQuery):
    ...
    for sym in watchlist:
        ...
        if price == 0:
            price = (ws.get_price(sym) if ws else 0.0) or await get_mid_price(sym)
        ...
        lines.append(
            f"🔹 <b>{sym}</b>: ${pretty_float(price, 4)} ({'🟢' if change_24h >= 0 else '🔴'} {change_24h:+.2f}%)\n"
            f"   F: {funding_rate*100:.4f}% ({funding_rate*24*365*100:.1f}% APR) | Vol: ${pretty_float(volume/1e6, 1)}M"
        )
    ...
    if buf:
        await call.message.edit_media(
            media=InputMediaPhoto(
                media=BufferedInputFile(buf.read(), filename="mo.png"),
                caption=f"📊 <b>Market Fundamentals ({sort_by.upper()})</b>",
                parse_mode="HTML"
            ),
...
async def cb_whales(call: CallbackQuery):
    ...
    text = (
        f"{_t(lang, 'whales_title')}\n\n"
        f"{_t(lang, 'whale_intro')}\n\n"
        f"{_t(lang, 'whale_alerts_on' if is_on else 'whale_alerts_off')}\n"
        f"{_t(lang, 'whale_watchlist_only_on' if wl_only else 'whales_all_assets')}\n"
...
async def cb_fear_greed(call: CallbackQuery):
    ...
    text = (
        f"{_t(lang, 'fng_title')}\n\n"
        f"{fng['emoji']} <b>{val}</b> — {fng['classification']}\n\n"
        f"<code>[{'█' * int(val/5) + '░' * (20 - int(val/5))}]</code>\n"
        f"<code>0   Fear         Greed   100</code>\n\n"
        f"{'📈' if change > 0 else ('📉' if change < 0 else '➖')} {_t(lang, 'fng_change', change=change)}\n\n"
...
async def cb_terminal(call: CallbackQuery):
    ...
    combined_assets, combined_positions = [], []
        ...
        if spot_bals:
            for b in spot_bals:
                ...
                if not entry or entry <= 0:
                    try:
                        fills = await db.get_fills_by_coin(wallet, coin_id)
                        entry = sum(float(f['px'])*float(f['sz']) for f in fills)/sum(float(f['sz']) for f in fills) if fills else 0.0
                    except Exception:
                        entry = 0.0
                if entry > 0 and px > 0:
