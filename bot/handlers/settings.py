...
async def cb_lang_menu(call: CallbackQuery):
    lang = await db.get_lang(call.message.chat.id)
    kb = InlineKeyboardBuilder(); kb.button(text="🇷🇺 Русский", callback_data="lang:ru"); kb.button(text="🇬🇧 English", callback_data="lang:en"); kb.button(text=_t(lang, "btn_back"), callback_data="cb_settings"); kb.adjust(2, 1)
    await smart_edit(call, _t(lang, "lang_title"), reply_markup=kb.as_markup())

@router.callback_query(F.data.startswith("lang:"))
async def cb_set_lang(call: CallbackQuery):
    await db.set_lang(call.message.chat.id, call.data.split(":")[1]); await cb_settings(call)
...
async def cb_wallets_menu(call: CallbackQuery):
    ...
    if not wallets:
        kb.button(text="➕ Add Wallet", callback_data="cb_add_wallet_prompt"); kb.button(text=_t(lang, "btn_back"), callback_data="cb_settings"); kb.adjust(1)
        await call.message.edit_text(_t(lang, "need_wallet"), reply_markup=kb.as_markup(), parse_mode="HTML"); return
    ...
    kb.button(text=_t(lang, "btn_back"), callback_data="cb_settings"); kb.adjust(1)
...
async def cb_del_wallet(call: CallbackQuery):
    await db.remove_wallet(call.message.chat.id, call.data.split(":")[1]); await cb_wallets_menu(call)

@router.callback_query(F.data == "cb_add_wallet_prompt")
async def cb_add_wallet_prompt(call: CallbackQuery):
    lang = await db.get_lang(call.message.chat.id); text = _t(lang, "add_wallet_prompt")
    await call.message.edit_text(text, reply_markup=_back_kb(lang, "cb_wallets_menu"), parse_mode="HTML"); await call.answer()
...
async def cmd_add_wallet(message: Message):
    ...
    if len(args) < 2: await message.answer(_t(lang, "add_wallet_usage"), parse_mode="HTML"); return
    wallet = args[1].lower()
    if not wallet.startswith("0x") or len(wallet) != 42:
        await message.answer(_t(lang, "invalid_wallet_addr"), parse_mode="HTML"); return
    wallets = await db.list_wallets(message.chat.id)
    if wallet not in wallets:
        if not await _ensure_billing_quota(message, message.chat.id, lang, "wallets", len(wallets), "billing_feature_wallets"): return
    await db.add_wallet(message.chat.id, wallet); ws = getattr(message.bot, "ws_manager", None)
    if ws: ws.track_wallet(wallet); await ws.subscribe_user(wallet)
...
async def cmd_tag(message: Message):
    ...
    if len(args) < 3: await message.answer(_t(lang, "tag_usage"), parse_mode="HTML"); return
    await db.update_wallet_settings(message.chat.id, args[1].lower(), tag=args[2]); await message.answer(_t(lang, "settings_updated"))
...
async def cb_set_prox_prompt(call: CallbackQuery, state: FSMContext):
    await state.update_data(menu_msg_id=call.message.message_id); await call.message.edit_text(_t(await db.get_lang(call.message.chat.id), "prox_input"), reply_markup=_back_kb(await db.get_lang(call.message.chat.id), "cb_wallets_alerts_menu"), parse_mode="HTML"); await state.set_state(SettingsStates.waiting_for_prox); await call.answer()

@router.callback_query(F.data == "set_vol_prompt")
async def cb_set_vol_prompt(call: CallbackQuery, state: FSMContext):
    await state.update_data(menu_msg_id=call.message.message_id); await call.message.edit_text(_t(await db.get_lang(call.message.chat.id), "vol_input"), reply_markup=_back_kb(await db.get_lang(call.message.chat.id), "cb_wallets_alerts_menu"), parse_mode="HTML"); await state.set_state(SettingsStates.waiting_for_vol); await call.answer()

@router.callback_query(F.data == "set_whale_prompt")
async def cb_set_whale_prompt(call: CallbackQuery, state: FSMContext):
    await state.update_data(menu_msg_id=call.message.message_id); await call.message.edit_text(_t(await db.get_lang(call.message.chat.id), "whale_input"), reply_markup=_back_kb(await db.get_lang(call.message.chat.id), "cb_wallets_alerts_menu"), parse_mode="HTML"); await state.set_state(SettingsStates.waiting_for_whale); await call.answer()
...
async def cb_flex_menu(call: CallbackQuery):
    lang = await db.get_lang(call.message.chat.id)
    if not await _ensure_billing_feature(call, call.message.chat.id, lang, "flex", "billing_feature_flex", is_callback=True): return
    kb = InlineKeyboardBuilder(); kb.button(text=_t(lang, "flex_period_day"), callback_data="cb_flex_gen:day"); kb.button(text=_t(lang, "flex_period_week"), callback_data="cb_flex_gen:week"); kb.button(text=_t(lang, "flex_period_month"), callback_data="cb_flex_gen:month"); kb.button(text=_t(lang, "flex_period_all"), callback_data="cb_flex_gen:all"); kb.button(text=_t(lang, "btn_back"), callback_data="cb_settings"); kb.adjust(2, 2, 1)
...
async def cb_flex_gen(call: CallbackQuery):
    lang = await db.get_lang(call.message.chat.id)
    if not await _ensure_billing_feature(call, call.message.chat.id, lang, "flex", "billing_feature_flex", is_callback=True): return
    period = call.data.split(":")[1]; await call.answer("Generating..."); wallets = await db.list_wallets(call.message.chat.id)
    if not wallets: await call.message.answer(_t(lang, "need_wallet")); return
    period_label = {"day": _t(lang, "flex_period_day"), "week": _t(lang, "flex_period_week"), "month": _t(lang, "flex_period_month"), "all": _t(lang, "flex_period_all")}.get(period, "Period")
    ...
    for wallet in wallets:
        ...
        equity_hist.sort(key=lambda x: x[0]); pnl_hist.sort(key=lambda x: x[0])
        if period == "all": p_start, e_start = 0.0, float(equity_hist[0][1]) - float(pnl_hist[0][1])
        else:
            closest_p = min(pnl_hist, key=lambda x: abs(x[0] - target_time)); p_start = float(closest_p[1])
            e_start = float(min(equity_hist, key=lambda x: abs(x[0] - closest_p[0]))[1])
        total_period_pnl += (float(pnl_hist[-1][1]) - p_start); total_start_equity += max(0, (float(equity_hist[-1][1]) - float(pnl_hist[-1][1])) if period == "all" else e_start); has_data = True
    if not has_data: await call.message.answer("❌ Not enough history data."); return
    from bot.analytics import prepare_account_flex_data; from bot.renderer import render_html_to_image
    try:
    ...
    except Exception as e: logger.error(f"Error rendering flex card: {e}"); await call.message.answer("❌ Error generating image.")
